$(document).ready(function () {
      ...
      success(data) {
        ...
        lista.forEach(row => {
          tabla += `
					<tr>
					<td>${row.fecha}</td>
					<td>${row.codigo_provedores}</td>     
        	<td><button type="button" class="btn btn-success  venta_detalle" id="${row.numero_compra}" data-bs-toggle="modal" data-bs-target="#detalleVenta">Productos</button></td>                 
					<td>${row.moton_t}</td>   
          <td class="opcciones d-flex justify-content-end">
          <button id="${row.numero_compra}"class="btn btn-danger borrar" data-bs-toggle="modal" data-bs-target="#Borrar"><img src="assets/img/basura.png"></button>
					</td>
					`
        })
        $('#tbody').html(tabla);
  ...
  $(document).on('click', '.venta_detalle', function () {
    ...
    $.post('', { detalleV: 'detV', id }, function (data) {
      ...
      $('#compraNombre').text(`Numero de Compra #${lista[0].numero_compra}.`);
